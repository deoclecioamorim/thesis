...
rand_design_1 <- as.matrix(plasma_design[,c('rand_intercept','rand_time_scaled','rand_time_squared_scaled')])

rand_design_0 <- as.matrix(plasma_design[,c('rand_intercept','rand_time_scaled')])
...
log_multivariate_t_1_reparm <- function(response,fixed_design,rand_design,parm,sample_size,repeated_measure)
{
  ...
  dmvt(response,mu,cov_mat,df=0.02,log=TRUE)
}
...
log_multivariate_t_0_reparm <- function(response,fixed_design,rand_design,parm,sample_size,repeated_measure)
{
  ...
  Z <- as.matrix(bdiag(mat_list))
  ## reparametization ####################
  lambda_mat <- matrix(c(exp(phi0),0,0,exp(phi1)),2,2)
  ########################################
  ...
  dmvt(x=response,mu,cov_mat,df=0.02,log=TRUE)
}
...
log_prior_1_reparm <- function(parm)
{
 ...
 # value <- log(dmvnorm(c(beta1,beta4,beta7,beta2,beta5,beta8,beta3,beta6,beta9),c(0,0,0,0,0,0,0,0,0),10*diag(9)) * dnorm(phi0,mean = log(0.3),sd = sqrt(2)) * dnorm(phi1,mean = log(0.3),sd = sqrt(2)) * dnorm(phi2,mean = log(0.3),sd = sqrt(2)) * dnorm(gamma12,0,1) * dnorm(gamma13,0,1) * dnorm(gamma23,0,1))
...
log_prior_0_reparm <- function(parm)
{
  ...
  #log(dmvnorm(c(beta1,beta4,beta7,beta2,beta5,beta8,beta3,beta6,beta9),c(0,0,0,0,0,0,0,0,0),10*diag(9)) * dnorm(phi0,mean = log(0.3),sd = sqrt(2)) * dnorm(phi1,mean = log(0.3),sd = sqrt(2)) * dnorm(gamma12,0,1))
  log(dmvnorm(c(beta1,beta4,beta7,beta2,beta5,beta8,beta3,beta6,beta9),c(0,0,0,0,0,0,0,0,0),10*diag(9)) * dmvnorm(c(phi0,phi1),c(log(0.3),log(0.3)),2*diag(2)) * dnorm(gamma12,0,1))
  }
...
#inits <- c(4,-0.8,0.16,4,-0.8,0.16,4,-0.8,0.16,0.5,-0.5,-2,-0.4,-0.8,0.4)
...
inits <- c(4,-0.72,0.16,4.3,-0.86,0.16,4.7,-0.94,0.16,0.2649602,-1.584746,-0.4)
#inits <- c(4,-0.8,0.16,4,-0.8,0.16,4,-0.8,0.16,0.5,-0.5,-0.4)
