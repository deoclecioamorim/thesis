...
rand_design_0 <- as.matrix(plasma_design[,c('rand_intercept')])
...
log_prior_1_reparm <- function(parm)
{
  ...
  value <- log(dmvnorm(c(beta1,beta4,beta7,beta2,beta5,beta8,beta3,beta6,beta9),c(0,0,0,0,0,0,0,0,0),10*diag(9)) * dmvnorm(c(phi0,phi1,phi2),c(log(0.3),log(0.3),log(0.3)),3*diag(3)) * dnorm(gamma12,0,1) * dnorm(gamma13,0,1) * dnorm(gamma23,0,1))
  value
}
...
log_prior_0_reparm <- function(parm)
{
  ...
  log(dmvnorm(c(beta1,beta4,beta7,beta2,beta5,beta8,beta3,beta6,beta9),c(0,0,0,0,0,0,0,0,0),10*diag(9)) * dnorm(phi0,mean = log(0.3),sd = sqrt(2)))
}
...
inits <- c(4,-0.72,0.16,4.3,-0.86,0.16,4.7,-0.94,0.16,0.1067385)
