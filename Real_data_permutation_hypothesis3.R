...
permute<-function(samples,fixed_effect,B,n)
{
  ...
  for(i in 1:B)
  {
    ...
    ## fit null model
    nullmod_hypo_3  <- lmer(plasma ~ group_A + group_B + group_C + time_A + time_B + time_C + time_squared_A + time_squared_B + time_squared_C - 1 + (1 + rand_time| Id),data=permute_samples,REML=FALSE)
    ## fit alternative model
    altermod_hypo_3 <- lmer(plasma ~ group_A + group_B + group_C + time_A + time_B + time_C + time_squared_A + time_squared_B + time_squared_C - 1 + (1 + rand_time + rand_time_squared| Id),data=permute_samples,REML=FALSE)
    ## likelihood ratio for observation
    logLik_permute <- as.numeric(-2*(logLik(nullmod_hypo_3) - logLik(altermod_hypo_3)))
...
func1<-function(plasma_design)
{
  ...
  nullmod_hypo_3  <- lmer(plasma ~ group_A + group_B + group_C + time_A + time_B + time_C + time_squared_A + time_squared_B + time_squared_C - 1 + (1 + rand_time| Id),data=plasma_design,REML=FALSE)
  altermod_hypo_3 <- lmer(plasma ~ group_A + group_B + group_C + time_A + time_B + time_C + time_squared_A + time_squared_B + time_squared_C - 1 + (1 + rand_time + rand_time_squared| Id),data=plasma_design,REML=FALSE)
  LME_logLik_obs <- as.numeric(-2*(logLik(nullmod_hypo_3) - logLik(altermod_hypo_3)))
  ...
  fixeff <- matrix(fixef(altermod_hypo_3),nrow=9,ncol=1)
  fixed_effects <- fixed_design %*% fixeff
  
  #remove rand effects
  raneff <- ranef(altermod_hypo_3)$Id
  
  
  rand_effect <- sapply(c(1:sample_size),function(i){plasma_design[plasma_design[,'Id'] == i,'rand_intercept'] * raneff[i,1] + plasma_design[plasma_design[,'Id'] == i,'rand_time'] * raneff[i,2]})
  
  plasma_design[,'plasma'] <- as.vector(sapply(c(1:sample_size),function(i){plasma_design[plasma_design[,'Id'] == i,'plasma'] - plasma_design[plasma_design[,'Id'] == i,'rand_intercept'] * raneff[i,1] - plasma_design[plasma_design[,'Id'] == i,'rand_time'] * raneff[i,2]}))
